...
| 激光雷达 DATA/TX | 3 | 输入 | P1 | 规范线序：实物 DATA/TX → GPIO3；当前现场默认 115200 8N1，诊断版可轮询 150000 等候选线序和波特率；若高于 3.3V 必须转换 |
| 激光雷达 CTL/RX | 43 | 输出 | P1 | 规范线序：ESP32 TX → 雷达 CTL/RX；上电/重启串口后发送 `A5 60` 启动 |
