...
const char* wiringLabelFor(int rx_pin, int tx_pin) {
  for (const LidarWiringCandidate& wiring : kLidarWirings) {
    if (wiring.rx_pin == rx_pin && wiring.tx_pin == tx_pin) {
      return wiring.label;
    }
  }
  return "?";
...
LidarBringupProbe::LidarBringupProbe(UartBus& uart, LidarEaiS2& lidar)
    : uart_(uart),
      lidar_(lidar),
      current_baud_(profile::LIDAR_UART_BAUD),
      active_rx_pin_(pins::PIN_LIDAR_RX),
      active_tx_pin_(pins::PIN_LIDAR_TX),
      active_wiring_label_(kLidarWirings[0].label) {}

void LidarBringupProbe::logBeginOk() {
  current_baud_ = uart_.baud();
  active_rx_pin_ = uart_.rxPin();
  active_tx_pin_ = uart_.txPin();
  active_wiring_label_ = wiringLabelFor(active_rx_pin_, active_tx_pin_);

  const size_t written = sendStartupSequence();
  FB_LOGI(
      "LIDAR begin: uart=%d wiring=%s rx=GPIO%d tx=GPIO%d baud=%lu "
      "startup=A560 auto_probe=wire+baud "
      "written=%u",
      pins::UART_NUM_LIDAR,
      active_wiring_label_ != nullptr ? active_wiring_label_ : "?",
      active_rx_pin_, active_tx_pin_,
      static_cast<unsigned long>(current_baud_),
...
void LidarBringupProbe::logBeginFailed() const {
  FB_LOGW("LIDAR begin failed: uart=%d rx=GPIO%d tx=GPIO%d baud=%lu",
          pins::UART_NUM_LIDAR, uart_.rxPin(), uart_.txPin(),
          static_cast<unsigned long>(uart_.baud()));
}

void LidarBringupProbe::clearInput() {
...
size_t LidarBringupProbe::sendStartupSequence() {
  ...
  clearInput();
  return uart_.write(kLidarStartCommand, sizeof(kLidarStartCommand));
}

void LidarBringupProbe::restartCandidate(uint8_t candidate_index,
                                         uint32_t now_ms,
                                         const char* reason) {
  if (!uart_.isEnabled()) {
    return;
  ...
  last_packets_ = 0;
  last_scans_ = 0;
  last_checksum_errors_ = 0;
  last_framing_errors_ = 0;
  healthy_logged_ = false;
  if (!uart_.restart(baud, wiring.rx_pin, wiring.tx_pin)) {
  ...
  probe_index_ = candidate_index;
  current_baud_ = baud;
  active_rx_pin_ = wiring.rx_pin;
  active_tx_pin_ = wiring.tx_pin;
  active_wiring_label_ = wiring.label;
  const size_t written = sendStartupSequence();
  FB_LOGW(
      "LIDAR probe candidate=%u/%u round=%u/%u wiring=%s "
      "rx=GPIO%d tx=GPIO%d baud=%lu reason=%s startup=A560 written=%u t=%lu",
      static_cast<unsigned>(probe_index_),
      static_cast<unsigned>(kLidarProbeCandidateCount - 1u),
      static_cast<unsigned>(probe_rounds_),
...
void LidarBringupProbe::probeNextCandidate(uint32_t now_ms,
                                           const char* reason) {
  if (probe_rounds_ >= kLidarMaxProbeRounds) {
    if (probe_index_ != 0) {
      restartCandidate(0, now_ms, "probe_exhausted_return_primary");
    }
    return;
  }

  uint8_t next =
      static_cast<uint8_t>((probe_index_ + 1u) % kLidarProbeCandidateCount);
  if (next == 0) {
    ++probe_rounds_;
  }
  restartCandidate(next, now_ms, reason);
}

void LidarBringupProbe::logDiagnostics(uint32_t now_ms) {
  if (!uart_.isEnabled()) {
    return;
  }

  if (last_diag_ms_ != 0 &&
      elapsedMs(now_ms, last_diag_ms_) < kLidarDiagPeriodMs) {
    return;
  }
  ...
  const uint32_t delta_rx = stats.rx_byte_count - last_rx_bytes_;
  const uint32_t delta_packets = stats.packet_count - last_packets_;
  const uint32_t delta_scans = stats.scan_count - last_scans_;
  const uint32_t delta_checksum_errors =
      stats.checksum_error_count - last_checksum_errors_;
  const uint32_t delta_framing_errors =
      stats.framing_error_count - last_framing_errors_;
  ...
  const LidarLineSample rx_line =
      log_line_evidence ? sampleLineLevel(active_rx_pin_) : LidarLineSample{};
  const LidarLineSample tx_line =
      log_line_evidence ? sampleLineLevel(active_tx_pin_) : LidarLineSample{};
  ...
  if (now_ms >= kLidarStartupGraceMs) {
    if (stats.rx_byte_count == 0) {
      ...
      if (last_start_retry_ms_ == 0 ||
          elapsedMs(now_ms, last_start_retry_ms_) >= kLidarRestartRetryMs) {
        last_start_retry_ms_ = now_ms;
        probeNextCandidate(now_ms, "no_rx_try_next_wire_or_baud");
      }
    } else if (stats.packet_count == 0) {
      const bool emit_raw_diag =
          last_raw_diag_ms_ == 0 ||
          elapsedMs(now_ms, last_raw_diag_ms_) >= kLidarRawDiagPeriodMs;
      const bool should_probe_stalled =
          lidar_stalled_without_packet &&
          (last_start_retry_ms_ == 0 ||
           elapsedMs(now_ms, last_start_retry_ms_) >= kLidarRestartRetryMs);
      ...
      if (delta_rx > 0 && stats.packet_count == 0) {
        probeNextCandidate(now_ms, "rx_without_known_header");
      } else if (should_probe_stalled) {
        last_start_retry_ms_ = now_ms;
        probeNextCandidate(now_ms, "rx_stalled_one_byte");
      }
      ...
      FB_LOGI(
          ...
          lidar.side_right_mm);
      healthy_logged_ = true;
    }
  }

  last_rx_bytes_ = stats.rx_byte_count;
  last_packets_ = stats.packet_count;
  last_scans_ = stats.scan_count;
  last_checksum_errors_ = stats.checksum_error_count;
  last_framing_errors_ = stats.framing_error_count;
}
